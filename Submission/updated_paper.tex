...
In particular, heatwaves which are characterised by high temperatures and stagnant meteorological conditions tend to have high ozone levels as was the case during the European heatwave in 2003 \citep{Solberg:2008, Vautard:2005}.
...
%remove to our knowledge and change sentence
...
These modelling studies did not consider the relationship between ozone, \chem{NO_x} and temperature. %confirm that this is the case
The review of \citet{Pusede:2015} also highlights a general lack of modelling studies looking at the relationship of ozone with temperature under different \chem{NO_x} conditions.
...
We compare the rate of increase of ozone with temperature obtained from the box model to both observations and regional model ouput. 
Additionaly, we look at the role of stagnation on the rate of increase of ozone with temperature.
...
Simulations were run for daylight hours in one full day using a latitude of $51$~\degree N, broadly representative of urban conditions in central Europe.
...
These reduced chemical mechanisms were chosen as they are currently used by modelling groups in 3D regional and global models \citep{Baklanov:2014}.
\citet{Coates:2015} described the implementation of these chemical mechanisms in MECCA.

The chemical mechanisms use temperature-dependent rate constants, \chem{k}(T), to represent temperature-dependent chemical processes.
These temperature-dependent chemical processes include the initial oxidation of VOC, peroxy nitrate (\chem{RO_2NO_2}) formation and destruction, and reactions between peroxy radicals and NO leading to alkyl nitrate (\chem{RONO_2}) formation.
However, not all chemical mechanisms represent the same chemical processes by a temperature-dependent rate constant. 
For example, in CB05, the rate constant of reactions \chem{RONO_2} formation during isoprene degradation is temperature dependent while \chem{RONO_2} formation during alkane degradation is temperature independent.
None of the chemical mechanisms represent the alkyl nitrate branching ratio as a temperature dependent process, \chem{RONO_2} chemistry is not well known \citep{Pusede:2015} although experiments have shown the temperature dependence of this branching ratio \citep{Atkinson:1987}.
\citet{Pusede:2015} described the influence that the temperature dependence of the \chem{RONO_2} branching ratio may have on ozone production and more work is required to adequately represent this process within chemical mechanisms.
...
Many types of VOCs are emitted from vegetation with isoprene and monoterpenes having globally the largest yearly emissions, $535$ and $162$~Tg~yr$^{-1}$ \citep{Guenther:2012}.
Temperature-dependent emissions of these highly-reactive BVOC in urban areas, characterised by high \chem{NO_x} emissions, during the summer months in urban areas leads to high levels of ozone pollution during the summertime, for example the study of \citet{Wang:2013} attributed high summertime levels of ozone in Taipei to increased isoprene emissions from vegetation during the hotter summer months.
The influence of BVOCs in urban areas has additional complexity as many BVOCs also react with ozone and stomatal uptake of ozone by vegetation are also sinks of ozone, the review of \citet{Calfapietra:2013} discusses the role of BVOCs in urban areas in more detail.

In all model runs, biogenic emissions of monoterpenes and isoprene are included with the model runs using a temperature-dependent source of BVOC emissions considered only the temperature-dependence of isoprene emissions as specified by MEGAN2.1 \citep{Guenther:2012}, Sect.~\ref{ss:megan} provides further details. 
We focused on the temperature-dependence of isoprene emissions as isoprene is the most important BVOC on the global scale \citep{Guenther:2006}, future work should assess the influence of temperature-dependent emissions of monoterpenes on ozone production especially to determine whether the increased emissions are a larger source or sink of ozone.
...
Our assumption that all anthropogenic VOC emissions are temperature dependent is not the case in reality where evaporative emissions increase with temperature \citep{Rubin:2006}, temperature dependent emission factors of anthropogenic emissions require detailed local knowledge of traffic fleets for example and our modelling study was not meant to focus on a particular region.
The influence of temperature dependent AVOC emissions on ozone production requires detailed knowledge of regional emissions and the strength of these sources, our box modelling study does not aim to characterise the influence of temperature dependent emissions of a particular area and thus was kept in a general state.
Furthermore, isoprene has be measured from vehicular exhausts in urban areas by \citet{Borbon:2001} and once again since we are not characterising the influence of temperature dependent emissions of a particular region the detailed knowledge of the strength of this source of isoprene, that is not included in the TNO-MACC\_III emission inventory.
Further modelling work assessing this influence on ozone production would be highly useful for mitigating ozone pollution in urban areas.
...
The idealised nature of our study was designed to be generally applicable to urban areas rather than performed detailed simulations representative of a particular region over a certain time frame.
Thus to directly compare the effects of temperature-dependent emissions of isoprene to temperature-dependent chemistry, the MEGAN2.1 parameters used to calculate isoprene emissions online by the model were chosen to give similar isoprene mixing ratios at $20$~\degree C to the temperature-independent emissions of isoprene in order to compare the effects of increased isoprene emissions with temperature.
...
The intermechanism differences between the ozone contours of Fig.~\ref{f:ozone_contours} obtained from the chemical mechanisms are not significant, the largest differences are produced at the highest temperatures and High-\chem{NO_x} conditions.
The maximum difference between ozone mixing ratios produced using the chemical mechanisms (at $40$~\degree C) in the High-\chem{NO_x} state (Fig.~\ref{f:O3-T}) is $10$~ppbv when using both a temperature-independent and temperature-dependent source of isoprene emissions.
These differences in ozone mixing ratios between the chemical mechanisms are similar to that obtained in \citet{Coates:2015} ($8$~ppbv) and \citet{vonSchneidemesser:2016} ($7$~ppbv).

The Maximal-O3 regime is reached with $\sim$~$30$~\% more \chem{NO_x} emissions using CB05 and RADM2 than the detailed MCMv3.2 chemical mechanism throughout the temperature range of our study and for both a temperature-independent and -dependent source of isoprene emissions.
The CRIv2 and MOZART-4 chemical mechanisms require very similar \chem{NO_x} emissions to the MCMv3.2 at each temperature to reach the maximum ozone production.
This indicates that the increase of ozone with temperature using CB05 and RADM2 is more sensitive to the faster reaction rates (temperature-independent emissions) and increased isoprene emissions (temperature-dependent emissions).
The larger slope of the increase of ozone with temperature (Table~\ref{t:mo3-t}) obtained with CB05 and RADM2 compared to the other chemical mechanisms is consistent with the conclusion that both these chemical mechanisms produce a more \chem{NO_x}-saturated state.
Thus when using CB05 and RADM2 for air quality studies, these chemical mechanisms would predict a higher sensitivity of ozone reduction to decreasing VOC-emissions.
The study using the 3D WRF-Chem model of \citet{Mar:2016} compared ozone levels produced using MOZART-4 and RADM2 and also noted that RADM2 is in a more \chem{NO_x}-saturated state than MOZART-4.
These differences in the sensitivity of ozone to \chem{NO_x} and VOC emissions using different chemical mechanisms highlight the need for air quality studies to assess the chemical scheme used by the model by performing simulations using different chemical mechanisms.

To re-cap, a very similar non-linear relationship between ozone, temperature and \chem{NO_x} is produced by each chemical mechanism with the absolute increase in ozone with temperature due to temperature-dependent chemistry larger than the increase in ozone with temperature due to temperature-dependent isoprene emissions.
...
The larger increases in ozone levels in the Maximal-O3 and High-\chem{NO_x} regimes indicate that strong reductions in \chem{NO_x} emissions are neccessary to offset the increase in ozone pollution at higher temperatures when isoprene emissions are higher.
...
Chemical reactions contributing to both production and consumption of \chem{O_x} have temperature-dependent rate constants, for example the production and decomposition of peroxy nitrates, thus to consider the effects of temperature-dependent reaction rates we analysed the production and consumption budgets of \chem{O_x}.
%merge with first sentence
The \chem{O_x} budgets are assigned to each \chem{NO_x} regime for each chemical mechanism and type of isoprene emissions.

The chemical reactions producing and consuming \chem{O_x} were determing and then allocated to major chemical categories in Fig.~\ref{f:ozone_budgets}. 
Production of \chem{O_x} is through the reactions of peroxy radicals with NO and in Fig.~\ref{f:ozone_budgets} the peroxy radicals are subdivided into `HO2', `RO2', `ARO2' representing the reactions of NO with \chem{HO2}, alkyl peroxy radicals and acyl peroxy radicals respectively.
Thus at each time step the total \chem{O_x} production rate is given by 
\begin{equation}
    \chem{PO_x} = k_{HO_2 + NO}[HO_2][NO] + \sum_i{k_{RO_{2,i} + NO}[RO_{2,i}][NO]} + \sum_j{k_{ARO_{2,j} + NO}[ARO_{2,j}][NO]}
\end{equation}
for each alkyl peroxy radical $i$ and acyl peroxy radical $j$ represented by each chemical mechanism.
The net contributions of peroxy nitrates, inorganic reactions and any other remaining organic reactions on the \chem{O_x} budget are represented by the `RO2NO2', `Inorganic' and `Other Organic' categories in Fig.~\ref{f:ozone_budgets}.
The net contributions of these categories to the \chem{O_x} budget was calculated by subtracting the cumulative day-time consumption rate from the production rate of the reactions contributing to each category.
For example, RO2NO2 produce \chem{O_x} when thermally decomposing or reacting with OH and consume \chem{O_x} when produced are produced from \chem{RO_2} reacting with \chem{NO_2}.
...
\end{equation}
for each peroxy nitrate species $k$ represented by each chemical mechanism.
In Fig.~\ref{f:ozone_budgets} the cumulative day-time budgets were calculated by summing the net contributions of the reaction rates of each category over the day-time period and normalised by the total chemical loss rate of the emitted VOC.

The absolute production and consumption budgets allocated to the major categories are displayed in Fig.~\ref{f:abs_Ox}
Both production and consumption \chem{O_x} budgets increase with temperature for each chemical mechanism and each \chem{NO_x} conditions and the overall net increase of \chem{O_x} production (white line in Fig.~\ref{f:abs_Ox}) is consistent with the increase in ozone mixing ratios for each panel in Fig.~\ref{f:O3-T}.
Moreover, the net chemical production of \chem{O_x} is larger when using a temperature-dependent source of isoprene emissions than a temperature-independent source of isoprene emissions, again this is consistent with Fig.~\ref{f:O3-T}.
%todo: include net production in absolute budgets, check that plotting is done correctly and include absolute and normalised in one plot. Separate the mixing one into different plot.
...
Our results indicate that the increased VOC reactivity and decomposition rate of peroxy nitrates are the temperature-dependent chemical processes leading to increased \chem{O_x} with temperature.
...
In our study, we considered the ozone production for a single day when considering longer timescale less-reactive VOC, such as alkanes, may also play a significant role in ozone production \citep{von:Schneidemesser:2016}.
Thus substituting emissions of more-reactive VOCs with those from less-reactive VOCs from anthropogenic activities could lead to a reduction in ozone production.

\subsection{Comparison to Observations and 3D Model Simulations} \label{ss:r_observations}
The final step in our study was to compare our idealised simulations to real-world observations, we wanted to see how our idealised setup could simulate the real-world relationship between ozone and temperature.
...
A further test was to include the output from a regional 3D model, 3D models are setup to simulate areas to investigate air quality over specific regions and include many more real-world processes (horizontal transport, stratospheric influence, changing meteorological parameters such as humidity, wind speed among others) that are not included in our box model setup.
...
Our idealised box model was setup to investigate the reasons for the observed increase of ozone with temperature and a direct comparison of the ozone mixing ratios produced from the simulations are thus difficult to directly compare side-by-side to both the observations and WRF-Chem output.
In particular, the ozone mixing ratios reported by observations and simulated using WRF-Chem are a function of more processes than included in our box model.
For example, VOCs are constantly emitted, \chem{NO_x} emissions have a diurnal profile, more realistic meterology processes such as the influence of low wind speed (when combined with warm temperatures give rise to stagnant conditions) whereas in the box model, VOC emissions are emitted as a $6$~hour pulse, \chem{NO} emissions are held constant and besides vertical mixing into a clean troposphere, there are no other transport processes.
Moreover, there are further differences between the box model and WRF-Chem such as how photolysis is treated and particle-phase chemistry is also utilised within WRF-Chem whereas our box model only considered gas-phase chemistry.
Despite these differences, it is interesting to compare the rate of change of ozone with temperature (m$_{\text{O3-T}}$) between the box model, observations and the more realistic WRF-Chem model.
This metric has been used by many studies %todo: include studies
and is calculated taking the linear slope of the increase of ozone with temperature.
Table~\ref{t:mo3-t} summarises the slopes (m$_{\text{O3-T}}$) of the linear fits of the box model simulations displayed in Fig.~\ref{f:comparison} in ppbv of ozone per \degree C determining the rate of increase of ozone with temperature.
...
The observational data produces an increase of $2.15$~ppbv/\degree C which is not as large as that reported for XX in XX and XX in XX.
...
As mentioned earlier, there are number of differences between our box model setup and both the observational and 3D model output and of these differences the one most relevant to our study could be the lack of stagnation.
...
Thus our box model study does not reproduce the rate of increase of ozone with temperature obtained from observational and 3D model simulations.
The reason for this discrepancy was that the box model did not represent stagnation conditions which are relevant to real-world conditions and in particular for simulating heatwaves.
The lack of mixing meant that the main chemical processes leading to ozone levels with increased temperature discussed in Sect.~\ref{ss:r_budgets}, namely increase oxidation of VOC and faster peroxy nitrate thermal decomposition, were able to proceed without the chemical reactants being transported away from the region.
This is particular important for times of heatwaves, indicating that stronger reductions in \chem{NO_x} and possibly VOCs during heatwaves are required to minimise the impact of increase ozone production at higher temperatures.
...
\begin{figure}[t] 
    ...
    \includegraphics[height=\textheight]{fig02}%
\end{figure}
...
\begin{figure}[ht]
    \centering
    \caption{Day-time budgets of \chem{O_x} allocated to the \chem{NO_x}-regimes allocated to the net contribution of reactions to \chem{O_x} budgets are allocated to categories of inorganic reactions, peroxy nitrates (RO2NO2), reactions of NO with HO2, alkyl peroxy radicals (RO2) and acyl peroxy radicals (ARO2). All other reactions are allocated to the 'Other Organic' category.}
    \subfloat[\chem{O_x} production and consumption budgets with box model setup including mixing.\label{f:abs_Ox}]{%
        \includegraphics[scale = 0.5]{fig05a}
    }
    \hfill
    \subfloat[\chem{O_x} production and consumption budgets with box model setup without including mixing.\label{f:abs_Ox_no_mix}]{%
        \includegraphics[scale = 0.5]{fig05b}
    }
\end{figure} 
