...
The relative difference in ozone mixing ratio produced using each chemical mechanism from the MCMv3.2 is shown in Fig.~\ref{f:ozone_diffs}.
...
These differences in peak ozone mixing producing from the reduced chemical mechanisms from the MCMv3.2 in each \chem{NO_X} condition is consistent with Fig.~\ref{f:O3-T} (described below) where RADM2 and CB05 produce generally produced higher ozone levels than the MCMv3.2.
...
The study of \citet{Mar:2016} compared ozone levels produced using the 3D WRF-Chem model with MOZART-4 and RADM2 and also noted that RADM2 is in a more VOC-sensitive state than MOZART-4.
These differences in the ozone production regime using different chemical mechanisms highlights the need for air quality studies to assess the chemical scheme used by the model, otherwise differing results from the same mitigation strategy may be produced.
