...
The modelling study of \citet{Vogel:1999} looked at the \chem{NO_y} transition value as an indicator of ozone sensitivity at different VOC and \chem{NO_x} conditions and investigated how this transition value changed with different meteorological conditions.
...
We compare the rate of increase of ozone with temperature obtained from the box model to both observations and regional model ouput and consider the role of stagnation on the rate of increase of ozone with temperature.
...
Experiments have shown the temperature dependence of the \chem{RONO_2} branching ratio for some VOCs \citep{Atkinson:1987} but generally \chem{RONO_2} chemistry is not well known \citep{Pusede:2015}.
...
Since isoprene is the most important BVOC on the global scale, we focused on the influence of the temperature-dependence of isoprene emissions on ozone levels.
...
Representing a temperature-dependent evaporative source of AVOC and an anthropogenic source of isoprene requires detailed local knowledge of these emission sources.
...
To directly compare the effects of temperature-dependent emissions of isoprene to temperature-dependent chemistry, the MEGAN2.1 parameters used to calculate isoprene emissions online by the model were chosen to give similar isoprene mixing ratios at $20$~\degree C to the temperature-independent emissions of isoprene.
...
Figure~\ref{f:ozone_contours} depicts the peak mixing ratio of ozone from each simulation as a function of the total \chem{NO_x} emissions and temperature when using a temperature-independent and temperature-dependent source of isoprene emissions for each chemical mechanism.
The relative difference in ozone mixing ratio produced using each reduced chemical mechanism from the MCMv3.2 is shown in %todo:Fig.~\ref{f:ozone_diffs}.
...
Regions of high temperatures and high \chem{NO_x} emissions in %todo:Fig.~\ref{f:tbc} 
leave to the largest inter-mechanism differences between ozone mixing ratios using reduced chemical mechanisms from the MCMv3.2 (up to $XXX$~ppbv).
...
The Maximal-O3 regime is reached with $\sim$~$30$~\% more \chem{NO_x} emissions using CB05 and RADM2 than the MCMv3.2 at each temperature for both a temperature-independent and temperature-dependent source of isoprene emissions.
...
The study using the 3D WRF-Chem model of \citet{Mar:2016} compared ozone levels produced using MOZART-4 and RADM2 and also noted that RADM2 is in a more VOC-sensitive state than MOZART-4.
These differences in the ozone production regime using different chemical mechanisms highlights the need for air quality studies to assess the chemical scheme used by the model.
...
Since chemical reactions contributing to both production and consumption of \chem{O_x} ($\equiv$ \chem{O_3 + NO_2 + O(^1D) + O}) have temperature-dependent rate constants, we analysed the production and consumption budgets of \chem{O_x} to consider the effects of temperature-dependent reaction rates on ozone production.
...
The net production or consumption of \chem{O_x} is also displayed in Fig.~\ref{f:budgets}.
...
Reactions of peroxy radicals with NO produce \chem{O_x} and the peroxy radicals are divided into `HO2', `RO2', `ARO2' representing the reactions of NO with \chem{HO_2}, alkyl peroxy radicals and acyl peroxy radicals respectively.
...
For example, \chem{RO_2NO_2} produce \chem{O_x} when thermally decomposing or reacting with OH and consume \chem{O_x} when produced by \chem{RO_2} reacting with \chem{NO_2}.
...
Both production and consumption \chem{O_x} budgets increase with temperature for each chemical mechanism and each \chem{NO_x} conditions. 
...
Over longer timescales, the contribution of less-reactive VOC, such as alkanes, may also significantly contribute to ozone production \citep{Butler:2011, vonSchneidemesser:2016}.
...
This metric has been used by many studies \citep{Sillman:1995a, Baertsch-Ritter:2004, Rasmussen:2013} and is calculated taking the linear slope of the increase of ozone with temperature.
...
The observational data produces an increase of $2.15$~ppbv/\degree C which is comparable to that of $2.2$~ppbv/\degree C obtained for the Northeast US after \chem{NO_x} reductions from power plants \citep{Rasmussen:2013} as well as that reported by \citet{Sillman:1995a} for the average rural US ($2.4$~ppbv/\degree C) and Milan, Italy ($2.8$~ppbv/\degree C, \citet{Baertsch-Ritter:2004}).
...
\begin{figure}[t] 
    ...
    \quad
    \subfloat[Percent difference in peak ozone from MCMv3.2 at each temperature and \chem{NO_x} levels. \label{f:ozone_diffs}]{%
