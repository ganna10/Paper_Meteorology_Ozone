...
In this study, we used an idealised box model with different chemical mechanisms (MCMv3.2, CRIv2, MOZART-4, RADM2, CB05) to examine the non-linear relationship between ozone, \chem{NO_x} and temperature, and compared this to previous observational studies.
...
The largest inter-mechanism differences were obtained at high temperatures and high-\chem{NO_x} emissions.
CB05 and RADM2 simulated more \chem{NO_x}-sensitive chemistry than MCMv3.2, CRIv2 and MOZART-4 which could lead to different mitigation strategies being proposed depending on the chemical mechanism.
...
Our simulations were designed as an idealised case and not to be exact representations of any particular place.
...
Laboratory experiments have shown the temperature dependence of the \chem{RONO_2} branching ratio for some VOCs \citep{Atkinson:1987} but generally \chem{RONO_2} chemistry is not well known \citep{Pusede:2015} and this level of detail is not represented by the chemical mechanisms.
Before chemical mechanisms can include the temperature-dependence of the \chem{RONO_2} branching ratio, further research is required.
...
Simulations were also performed to assess the role of mixing on the increase of ozone with temperature.
...
Thus, these simulations approximate stagnant conditions that favour accumulation of secondary VOC oxidation products and enhanced ozone production.
...
These differences in peak ozone mixing ratio produced from the reduced chemical mechanisms compared with the MCMv3.2 in each \chem{NO_x} condition are consistent with Fig.~3 (described below) where RADM2 and CB05 generally produced higher ozone levels than the MCMv3.2.
Also consistent with Fig.~3, CRIv2 produced the most similar amounts of ozone to the MCMv3.2 in each \chem{NO_x} condition whereas MOZART-4 tended to produce lower ozone mixing ratios than the MCMv3.2 in High-\chem{NO_x} conditions.
...
The \chem{NO_x} emissions required for maximum ozone production (the contour ridges in Fig.~\ref{f:ozone_contours}) at each temperature is displayed in Fig.~1 of the supplementary material.
This figure illustrates that RADM2 and CB05 require higher \chem{NO_x} emissions than the MCMv3.2 to achieve maximum ozone production at each temperature for both a temperature-independent and temperature-dependent source of isoprene emissions.
At $20$~\degree C, maximum ozone production is reached with $\sim$~$30$~\% more \chem{NO_x} emissions using CB05 and RADM2 than the MCMv3.2 with a temperature-independent and temperature-dependent source of isoprene emissions.
...
Thus when modelling the air quality over a particular region using RADM2 and CB05, these mechanisms would be expected to simulate more \chem{NO_x}-sensitive chemistry and a lower increase of ozone with temperature than the MCMv3.2, CRIv2 and MOZART-4 chemical mechanisms for the same conditions (i.e.~emissions, meteorology and radiation).
...
These results are consistent between each chemical mechanism, although for the same \chem{NO_x} and VOC conditions RADM2 and CB05 simulate a more \chem{NO_x}-sensitive regime at the same temperature than the other chemical mechanisms (MCMv3.2, CRIv2, MOZART-4).
...
Our results indicate that increased VOC reactivity due to faster rate constants for the reaction with OH and the decomposition rate of peroxy nitrates are the temperature-dependent chemical processes leading to increased production of \chem{O_x} with temperature.
Out of these two chemical processes, the increased VOC reactivity with OH with temperature had a larger influence on the increase of \chem{O_x} production with temperature.
...
Moreover, the \chem{O_x} budgets normalised by the chemical loss rate of VOC for the simulations without mixing show no appreciable difference to the simulations including mixing.
...
ehus during stagnant conditions, stronger reductions in \chem{NO_x} are required to minimise the impact of increased ozone production at higher temperatures on the urban population.
...
Simulations with high-\chem{NO_x} emissions at high temperatures led to the largest differences in ozone mixing ratios predicted by the different chemical mechanisms, future work is needed to address these inter-mechanism differences.
Our results indicated that CB05 and RADM2 simulated more \chem{NO_x}-sensitive chemistry than MCMv3.2, CRIv2 and MOZART-4.
Thus for the same conditions, CB05 and RADM2 would simulate a lower increase of ozone with temperature than MCMv3.2, CRIv2 and MOZART-4 which could lead to different mitigation strategies being proposed depending on the chemical mechanism.
...
The authors would like to thank Noelia Otero Felipe for assistance with the processing of the ERA-Interim data.
