...
Laboratory experiments have shown the temperature dependence of the }\chem{RONO_2} \DIFadd{branching ratio for some VOCs }\citep{Atkinson:1987} \DIFadd{but generally }\chem{RONO_2} \DIFadd{chemistry is not well known }\citep{Pusede:2015}\DIFadd{.
Further work is required before including an adequate description of the temperature-dependence of the }\chem{RONO_2} \DIFadd{branching ratio is included by chemical mechanisms.}\DIFaddend 
...
These differences in peak ozone mixing produced from the reduced chemical mechanisms from the MCMv3.2 in each }\chem{NO_x} \DIFadd{condition is consistent with Fig.~\ref{f:O3-T} (described below) where RADM2 and CB05 generally produced higher ozone levels than the MCMv3.2.
Also consistent with Fig.~\ref{f:O3-T}, CRIv2 produced the most similar amounts of ozone to the MCMv3.2 in each }\chem{NO_x} \DIFadd{condition whereas MOZART-4 tended to produce lower ozone mixing ratios than the MCMv3.2 in High-}\chem{NO_x} \DIFadd{conditions.
...
\DIFadd{Maximum ozone production (the contour ridges in Fig.~\ref{f:ozone_contours}) is reached with $\sim$~$30$~}\% \DIFadd{more }\chem{NO_x} \DIFadd{emissions using CB05 and RADM2 than the MCMv3.2 at each temperature for both a temperature-independent and temperature-dependent source of isoprene emissions.
...
The study of }\citet{Mar:2016} \DIFadd{compared ozone levels produced using the 3D WRF-Chem model with MOZART-4 and RADM2 and also noted that RADM2 is in a more VOC-sensitive state than MOZART-4 for the same simulated conditions.
These differences in the ozone production regime using different chemical mechanisms highlight the need for air quality studies to assess the chemical scheme used by the model, otherwise differing results from the same mitigation strategy may be produced.}\DIFaddend 
...
However, over longer timescales, the contribution of less-reactive VOC, such as alkanes, may also significantly contribute to ozone production }\citep{Butler:2011, vonSchneidemesser:2016} \DIFadd{and should be considered as part of a mitigation strategy for AVOC emissions.}
