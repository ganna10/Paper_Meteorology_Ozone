...
Simulations were \DIFdelbegin \DIFdel{broadly representative of urban conditions in central Europe and were }\DIFdelend run for daylight hours in one full day \DIFaddbegin \DIFadd{using a latitude of $51$~}\degree \DIFadd{N, broadly representative of conditions in central Europe}\DIFaddend .
...
The model results (\url{http://cwf.iass-potsdam.de/}) at the 700\DIFaddbegin \DIFadd{~}\DIFaddend hPa height were chosen and the daily average was used as input into the boxmodel.  
...
\DIFaddbegin \citet{Coates:2015} \DIFadd{described the implementation of these chemical mechanisms in MECCA.}\DIFaddend 

The chemical mechanisms use temperature-dependent rate constants, \chem{k}(T), to represent temperature-dependent chemical processes\DIFdelbegin \DIFdel{.
These temperature-dependent chemical processes include }\DIFdelend \DIFaddbegin \DIFadd{, including }\DIFaddend the initial oxidation of VOC, peroxy nitrate (\chem{RO_2NO_2}) formation and destruction, and reactions between peroxy radicals and NO leading to alkyl nitrate (\chem{RONO_2}) formation.
...
\DIFaddbegin \DIFadd{Furthermore, none of the chemical mechanisms in our study represent the }\chem{RONO_2} \DIFadd{branching ratio as a temperature dependent process. 
Laboratory experiments have shown the temperature dependence of the }\chem{RONO_2} \DIFadd{branching ratio for some VOCs }\citep{Atkinson:1987} \DIFadd{but generally }\chem{RONO_2} \DIFadd{chemistry is not well known }\citep{Pusede:2015}\DIFadd{.}\DIFaddend 
...
Model runs using a temperature-dependent source of BVOC emissions considered only the temperature-dependence of isoprene emissions as specified by }\DIFaddend MEGAN2.1 \citep{Guenther:2012}\DIFdelbegin \DIFdel{specified the temperature-dependent BVOC emissions of isoprene}\DIFdelend , Sect.~\ref{ss:megan} provides further details. 
...
Further modelling work assessing the influence of these temperature-dependent emission sources on ozone production would be useful for mitigating ozone pollution in urban areas}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{To directly compare the effects of temperature-dependent emissions of isoprene to temperature-dependent chemistry, the }\DIFaddend MEGAN2.1 parameters \DIFaddbegin \DIFadd{used to calculate isoprene emissions online by the model }\DIFaddend were chosen to give similar isoprene mixing ratios at $20$~\degree C to the temperature-independent emissions of isoprene\DIFdelbegin \DIFdel{in order to compare the effects of increased isoprene emissions with temperature}\DIFdelend .
...
\DIFaddbegin \DIFadd{The relative difference in ozone mixing ratio produced using each chemical mechanism from the MCMv3.2 is shown in Fig.~\ref{f:ozone_diffs}.
...
\DIFadd{Regions of high temperatures and high }\chem{NO_x} \DIFadd{emissions in Fig.~\ref{f:ozone_diffs} generally lead to the largest inter-mechanism differences between ozone mixing ratios using reduced chemical mechanisms from the MCMv3.2 (up to $13$~}\%\DIFadd{).
These differences in peak ozone mixing producing from the reduced chemical mechanisms from the MCMv3.2 in each }\chem{NO_X} \DIFadd{condition is consistent with Fig.~\ref{f:O3-T} (described below) where RADM2 and CB05 produce generally produced higher ozone levels than the MCMv3.2.
Also consistent with Fig.~\ref{f:O3-T}, CRIv2 produced the most similar amounts of ozone to the MCMv3.2 in each }\chem{NO_x} \DIFadd{condition whereas MOZART-4 tended to produced lower ozone mixing ratios than the MCMv3.2 in High-}\chem{NO_x} \DIFadd{conditions.
In Fig.~\ref{f:O3-T}, a maximum difference of $10$~ppbv between ozone mixing ratios produced using the chemical mechanisms is at $40$~}\degree \DIFadd{C in the High-}\chem{NO_x} \DIFadd{state when using both a temperature-independent and temperature-dependent source of isoprene emissions.
These differences in ozone mixing ratios between the chemical mechanisms are similar to that obtained by }\citet{Coates:2015} \DIFadd{($8$~ppbv) and }\citet{vonSchneidemesser:2016} \DIFadd{($7$~ppbv).}

\DIFadd{The Maximal-O3 regime is reached with $\sim$~$30$~}\% \DIFadd{more }\chem{NO_x} \DIFadd{emissions using CB05 and RADM2 than the MCMv3.2 at each temperature for both a temperature-independent and temperature-dependent source of isoprene emissions.
...
Thus when modelling the air quality over a particular region using RADM2 and CB05, these mechanisms would predict a faster increase of ozone with temperature (i.e. more VOC-sensitive ozone production).
...
The study of }\citet{Mar:2016} \DIFadd{compared ozone levels produced using the 3D WRF-Chem model with MOZART-4 and RADM2 and also noted that RADM2 is in a more VOC-sensitive state than MOZART-4.
These differences in the ozone production regime using different chemical mechanisms highlights the need for air quality studies to assess the chemical scheme used by the model, otherwise differing results from the same mitigation strategy may be produced.}\DIFaddend 
...
These results are consistent between each chemical mechanism, although RADM2 and CB05 simulate a more VOC-sensitive regime for the same temperature range than the other chemical mechanisms (MCMv3.2, CRIv2, MOZART-4).
...
\DIFadd{Since chemical reactions contributing to both production and consumption of }\DIFaddend \chem{O_x} ($\equiv$ \chem{O_3 + NO_2 + O(^1D) + O}) \DIFdelbegin \DIFdel{normalised by the total chemical loss rate of the emitted VOC (Fig.~\ref{f:ozone_budgets})}\DIFdelend \DIFaddbegin \DIFadd{have temperature-dependent rate constants, we analysed the production and consumption budgets of }\chem{O_x} \DIFadd{to determine the temperature-dependent chemical processes controlling the increase of ozone with temperature in fig.~\ref{f:O3-T}}\DIFaddend .
The \chem{O_x} budgets \DIFaddbegin \DIFadd{displayed in Fig.~\ref{f:budgets} }\DIFaddend are assigned to each \chem{NO_x} regime for each chemical mechanism and \DIFdelbegin \DIFdel{type }\DIFdelend \DIFaddbegin \DIFadd{source }\DIFaddend of isoprene emissions.
...
As the production efficiency of \chem{O_x} remains constant with temperature ($\sim2$~molecules of \chem{O_x} per molecule of VOC oxidised, Fig.~\ref{f:ozone_budgets}), the rate of \chem{O_x} production is controlled by the oxidation \DIFaddbegin \DIFadd{rate }\DIFaddend of VOCs.
...
\DIFaddbegin \DIFadd{Our results indicate that increased VOC reactivity and decomposition rate of peroxy nitrates are the temperature-dependent chemical processes leading to increased }\chem{O_x} \DIFadd{with temperature.
...
The study of }\citet{Li:2014} \DIFadd{also showed that reactive VOC such as ethene and xylene were responsible for ozone formation in Asia.
Besides decreasing }\chem{NO_x} \DIFadd{emissions to address increased ozone pollution at higher temperatures, assessing the speciation of AVOC emissions and in particular the reactivity of the AVOC may also pay dividends for improved air quality,
However, over longer timescales, the contribution of less-reactive VOC, such as alkanes, may also significantly contribute to ozone production }\citep{Butler:2011, vonSchneidemesser:2016}\DIFadd{.}
...
Firstly, we compared the box model simulated to }\DIFaddend the interpolated observations of the maximum daily 8~h mean (MDA8) of ozone from \citet{Schnell:2015} and the meteorological observational data set of the ERA-Interim re-analysis\DIFaddbegin \DIFadd{.
...
\DIFdelbegin \DIFdel{Model output from the 3D WRF-Chem regional model using MOZART-4 chemistry set-up over the European domain for simulations of the year 2007 from }%DIFDELCMD < \citet{Mar:2016} %%%
\DIFdel{was used to further }\DIFdelend \DIFaddbegin \DIFadd{A further test was to }\DIFaddend compare the box model simulations to \DIFdelbegin \DIFdel{a model including more meteorological processes than }\DIFdelend \DIFaddbegin \DIFadd{the output from a regional 3D model as 3D models include more real-world processes influencing ozone production not included in }\DIFaddend our box model.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{We used output from the WRF-Chem 3D model set-up over the European domain to simulate ozone production in the year 2007 using MOZART-4 chemistry, further details are described in }\citet{Mar:2016}\DIFadd{.
}\DIFaddend Figure~\ref{f:comparison} compares the observational and WRF-Chem data from summer 2007 averaged over central and eastern Germany, where summertime ozone values are driven by temperature \citep{Otero:2016}, to the MDA8 values of ozone from the box model simulations for each chemical mechanism (solid lines).
\DIFaddbegin 

\DIFadd{Directly comparing the ozone mixing ratios from our idealised box model simulations to the WRF-Chem output is difficult due to significant differences in the setups of the models.
...
\DIFadd{The linear slope of the observational data indicates an increase of $2.15$~ppbv ozone per }\degree \DIFadd{C, this is comparable to the increase of ozone with temperature from other recent studies over urban areas: $2.2$~ppbv/}\degree \DIFadd{C obtained over the Northeast US }\citep{Rasmussen:2013} \DIFadd{and Milan, Italy ($2.8$~ppbv/}\degree \DIFadd{C, }\citet{Baertsch-Ritter:2004}\DIFadd{).
}\DIFaddend Despite a high bias in simulated ozone in WRF-Chem, the rate of change of ozone with temperature from the WRF-Chem simulations ($2.05$~ppbv/\degree C) is similar to the rate of change of ozone with temperature from the observed data ($2.15$~ppbv/\degree C).
...
This box model setup did not consider stagnant atmospheric conditions }\DIFdelend \DIFaddbegin \DIFadd{Out of the metereological conditions not represented by our box model, stagnation is the most likely cause of the lower sensitivity of m$_{\text{O3-T}}$ in the box model compared to the observations and WRF-Chem output.
Stagnant atmospheric conditions are }\DIFaddend characteristed by low wind speeds slowing the transport of ozone and its precursors away from sources \DIFdelbegin \DIFdel{.
Stagnant conditions }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend have been correlated to high-ozone episodes in the summer over eastern US \citep{Jacob:1993}.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In order to investigate the sensitivity of ozone production to mixing, \DIFaddbegin \DIFadd{further }\DIFaddend box model simulations were performed without vertical mixing to approximate stagnant conditions that favour accumulation of secondary VOC oxidation products.
...
\DIFdelbegin \DIFdel{Table~\ref{t:mo3-t} summarises the slopes (m$_{\text{O3-T}}$) of the linear fits of the box model simulations displayed in Fig.~\ref{f:comparison} in ppbv of ozone per }%DIFDELCMD < \degree %%%
\DIFdel{C determining the rate of increase of ozone with temperature, for both case: with, and without mixing.}\DIFdelend 
...
Moreover, the }\chem{O_x} \DIFadd{budgets }\DIFaddend normalised by the chemical loss rate of VOC \DIFdelbegin \DIFdel{remains unchanged.Furthermore, }\DIFdelend \DIFaddbegin \DIFadd{for the simulations without mixing show no appreciable difference to the simulations including mixing (Fig.~\ref{f:ozone_budgets}).
...
Thus during heatwaves, stronger reductions in }\chem{NO_x} \DIFadd{are required to minimise the impact of increased ozone production at higher temperatures on the urban population.}
